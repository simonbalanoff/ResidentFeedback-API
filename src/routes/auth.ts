...
authRouter.post("/refresh", async (req, res) => {
  ...
  if (!doc) return res.status(401).json({ error: "invalid" });
  try {
...
authRouter.get("/me", auth(["surgeon", "admin"]), async (req, res) => {
  const user = await User.findById((req as any).user.sub).lean();
...
authRouter.post("/reset", async (req, res) => {
  ...
  if (!pr) return res.status(400).json({ error: "invalid" });
...
async function bcrypthash(p: string) {
